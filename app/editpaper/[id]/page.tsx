...
export default function EditPaperPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (res.data) {
          ...
          if (data.style) {
            setStyles((prev) => ({ 
                ...prev, 
                ...data.style, // Database se saved styles load ho rahe hain
            }));
          }
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Payload mein styles (including columns) bhej rahe hain
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-bold text-slate-600 animate-pulse">Loading Editor...</div>;
  ...
  return (
    <div className="relative flex h-screen w-screen bg-slate-100 overflow-hidden font-sans text-black">
      ...
      <aside className="w-64 bg-white border-r flex flex-col shadow-xl z-20 print:hidden text-black">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b">
            ...
            {openSection === "info" && (
              <div className="px-5 pb-5 pt-1 bg-slate-50/50 space-y-4">
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase">Examination Date</label>
                  <input type="date" value={paperData?.paperDate || ""} onChange={(e) => setPaperData({...paperData, paperDate: e.target.value})} className="w-full p-2 border rounded text-xs text-gray-600 outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase">Time Allowed</label>
                  <input type="text" placeholder="e.g. 2 Hours" value={paperData?.paperTime || ""} onChange={(e) => setPaperData({...paperData, paperTime: e.target.value})} className="w-full p-2 border rounded text-xs text-gray-600 outline-none" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="border-b">
            ...
            {openSection === "layouts" && (
              <div className="px-5 pb-5 pt-1 bg-slate-50/50 grid grid-cols-2 gap-2">
                {layouts.map((layout) => (
                  <button key={layout.id} onClick={() => setStyles({...styles, layoutType: layout.id})} className={`py-2 px-3 text-[10px] font-bold uppercase border rounded-lg transition-all ${styles.layoutType === layout.id ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-200 bg-white text-slate-500'}`}>
                    {layout.name}
                  </button>
                ))}
              </div>
              ...
              <div className="px-5 pb-5 pt-1 bg-slate-50/50 space-y-4">
                 <div className="space-y-1">
                    ...
                    <div className="flex rounded-md">
                      <button onClick={()=>setStyles({...styles, fontFamily: "font-sans"})} className={`flex-1 py-1 text-[10px] border rounded-l-md ${styles.fontFamily==='font-sans' ? 'bg-blue-600 text-white' : 'bg-white'}`}>Sans</button>
                      <button onClick={()=>setStyles({...styles, fontFamily: "font-serif"})} className={`flex-1 py-1 text-[10px] border rounded-r-md ${styles.fontFamily==='font-serif' ? 'bg-blue-600 text-white' : 'bg-white'}`}>Serif</button>
                    </div>
                 </div>
                 <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                       <label className="text-[9px] font-bold text-slate-400 uppercase">Ques Size</label>
                       <input type="number" value={styles.headingSize} onChange={(e)=>setStyles({...styles, headingSize: e.target.value})} className="w-full p-1.5 border rounded text-xs" />
                    </div>
                    <div className="space-y-1">
                       <label className="text-[9px] font-bold text-slate-400 uppercase">Text Size</label>
                       <input type="number" value={styles.textSize} onChange={(e)=>setStyles({...styles, textSize: e.target.value})} className="w-full p-1.5 border rounded text-xs" />
                    </div>
                 </div>
              </div>
            )}
            ...
            {openSection === "elements" && (
              <div className="px-5 pb-5 pt-1 bg-slate-50/50 space-y-4">
                 <div className="space-y-2 bg-white p-2.5 rounded-lg border shadow-sm">
                    <span className="text-[10px] font-black text-slate-700 uppercase">Question Layout</span>
                 ...
                 <div className="flex items-center justify-between bg-white p-2.5 rounded-lg border">
                    <span className="text-[10px] font-bold text-slate-700">Bubble Sheet</span>
        ...
        <div className="p-4 bg-white border-t space-y-2">
          <button onClick={() => window.print()} className="w-full py-2.5 bg-slate-800 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg">
            <HiOutlinePrinter size={16} /> PRINT PAPER
          </button>
          <button onClick={handleSave} disabled={isSaving} className="w-full py-3 bg-blue-600 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg">
      ...
      {/* MAIN PAPER AREA */}
      <main className="flex-1 overflow-y-auto p-8 flex justify-center bg-slate-200 print:p-0 print:bg-white">
        <div 
          ...
          {paperData && (
            <div className="relative z-10">
              ...
              {styles.showBubbleSheet && paperData.mcqBatches?.length > 0 && (
                <div className="border border-slate-300 p-2 rounded-xl mt-4">
                    <div className="grid grid-cols-4 gap-4">
                      {paperData.mcqBatches[0].questions.map((_:any, i:number) => (
                          <div key={i} className="flex items-center gap-2 justify-center">
                             ...
                             <div className="flex gap-1">
                               {['A','B','C','D'].map((opt) => (
                                   <div key={opt} className="w-4 h-4 rounded-full border flex items-center justify-center text-[8px]">{opt}</div>
                               ))}
                             </div>
                          </div>
                      ))}
              ...
              <div style={{ fontSize: styles.textSize + "px" }} className="mt-6">
                {/* MCQs */}
                      ...
                      {batch.questions.map((q: any, i: number) => (
                        <div key={i}>
                          <div className="flex gap-2">
                             ...
                             <AutoResizeTextarea value={q.question} onChange={(e: any) => updateQuestion('mcqs', bIdx, i, e.target.value)} className="font-bold" style={{ fontSize: styles.headingSize + "px", color: styles.textColor }} />
                          </div>
                          <div className="grid grid-cols-4 gap-4 mt-2 ml-6">
                            {q.options && Object.entries(q.options).map(([k, v]: any) => (
                              <div key={k} className="flex items-center gap-2">
                                ...
                                <input type="text" value={v as string} onChange={(e) => updateOption(bIdx, i, k, e.target.value)} className="bg-transparent outline-none w-full" />
                              </div>
                            ))}
                          </div>
                        </div>
                  ...
                  <div className="mt-8">
                    ...
                    {paperData.shortBatches.map((batch: any, bIdx: number) => (
                      <div key={bIdx} className="mb-6">
                        <div className="flex justify-between items-center bg-slate-50 p-1 mb-3 px-3 border-l-4" style={{ borderLeftColor: styles.textColor }}>
                          <span className="text-[11px] font-black uppercase">Q. No {bIdx + 2}: Attempt any {batch.config.attempt} out of {batch.config.total}.</span>
                        ...
                        <div className={`grid ${styles.shortCols === 2 ? 'grid-cols-2 gap-x-8 gap-y-4' : 'grid-cols-1 gap-y-4'} ml-4`}>
                           {batch.questions.map((q: any, i: number) => (
                              <div key={i} className="flex gap-2">
                                 <span className="font-bold shrink-0">({i+1})</span>
                                 <AutoResizeTextarea value={q.question} onChange={(e: any) => updateQuestion('shorts', bIdx, i, e.target.value)} className="font-bold" style={{ fontSize: styles.headingSize + "px", color: styles.textColor }} />
                              </div>
                           ))}
                        </div>
                      </div>
                ...
                {paperData.longBatches?.length > 0 && (
                  <div className="mt-10">
                    ...
                    {paperData.longBatches.map((batch: any, bIdx: number) => (
                      <div key={bIdx} className="mb-6">
                        <div className="flex justify-between items-center bg-slate-50 p-1 mb-3 px-3 border-l-4" style={{ borderLeftColor: styles.textColor }}>
                          <span className="text-[11px] font-black uppercase">Attempt any {batch.config.attempt} out of {batch.config.total}.</span>
                        </div>
                        <div className={`grid ${styles.longCols === 2 ? 'grid-cols-2 gap-x-8 gap-y-6' : 'grid-cols-1 gap-y-6'} ml-4`}>
                           {batch.questions.map((q: any, i: number) => (
                              <div key={i} className="flex gap-2">
                                 ...
                                 <AutoResizeTextarea value={q.question} onChange={(e: any) => updateQuestion('longs', bIdx, i, e.target.value)} className="font-bold" style={{ fontSize: styles.headingSize + "px", color: styles.textColor }} />
                              </div>
                           ))}
                        </div>
                      </div>

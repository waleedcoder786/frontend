...
export default function DashboardPage() {
  ...
  const filteredStats = useMemo(() => {
    const allStats = [
      { label: 'Generate Paper', value: savedPapers.length, color: 'bg-blue-500', lightColor: 'bg-blue-100/50', icon: <FaPlus />, path: '/generate-paper' },
      { label: 'Saved Papers', value: savedPapers.length, color: 'bg-emerald-500', lightColor: 'bg-emerald-100/50', icon: <FaSave />, path: '/saved-papers' },
      { label: 'Past Papers', value: 'Punjab', color: 'bg-purple-500', lightColor: 'bg-purple-100/50', icon: <FaHistory />, path: '/past-papers' },
      { label: 'Total Teachers', value: savedTec.length, color: 'bg-indigo-500', lightColor: 'bg-indigo-100/50', icon: <FaChalkboardTeacher />, path: '/teachers' },
      { label: 'Paper History', value: '0', color: 'bg-cyan-500', lightColor: 'bg-cyan-100/50', icon: <FaFileAlt />, path: '/paper-history' },
      { label: 'Users', value: allUsers.length, color: 'bg-slate-700', lightColor: 'bg-slate-200/50', icon: <FaUsers />, path: '/users' },
      { label: 'Add Data', value: "DB", color: 'bg-orange-500', lightColor: 'bg-orange-100/50', icon: <PlusCircle />, path: '/add-data' },
    ];
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className={`text-2xl sm:text-4xl font-black text-slate-800 tracking-tighter group-hover:${stat.color.replace("bg-", "text-")} transition-colors`}>
                        {stat.value}
                      </span>
                    </div>
                  </div>

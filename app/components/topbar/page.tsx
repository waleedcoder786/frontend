...
function Page() {
    ...
    const [user, setUser] = useState({
        ...
        email: '',
        profilePic: '',
        password: '' // This stores the hashed password from DB
    });
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const searchRes = await axios.get(`http://localhost:3001/users?email=${user.email}`);
            ...
            if (searchRes.data.length > 0) {
                ...
                // 5. Update Database
                const response = await axios.put(`http://localhost:3001/users/${userId}`, updatedData);
...
<div className="relative bg-white w-full max-w-md rounded-xl shadow-2xl p-8 animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto no-scrollbar">                        <div className="flex justify-between items-center mb-2">
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            ...
                            <div className="flex flex-col items-center py-1">
                                <div className="w-24 h-24 rounded-3xl bg-slate-100 overflow-hidden border-4 border-slate-50 shadow-inner mb-2">
                                    {user.profilePic ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center text-3xl text-slate-300 font-bold uppercase">{user.name?.charAt(0)}</div>
                                    )}
                                </div>
                                <input 
                                    type="text" 
                            ...
                            {/* <div className="space-y-1 pt-2 border-t border-slate-100"> */}
                                <div>
                                    <label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Verify Current Password</label>
                                    <input 
                                        type="password" 
